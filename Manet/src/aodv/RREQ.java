...
public class RREQ {
	...
	// RREQメッセージの送信
	// 引数：送信先(String型)
	public void send(byte[] str ,boolean flagJ ,boolean flagR ,boolean flagG ,boolean flagD ,boolean flagU
			,int toSeq ,int fromSeq,int ID,int TTL) throws Exception{
		...
		fromIpAdd = InetAddress.getLocalHost().getAddress();
		fromSeqNum = fromSeq;
		timeToLive = TTL;
...
/******************************************************************/
	...
	/***** RREQメッセージの転送 *****/
	public void send2(byte[] data) throws Exception{
	    ...
	    // UDPパケット
	    DatagramPacket sendPacket =
	        new DatagramPacket(data, 28, remoteAddress);
	...
	// 引数：RREQメッセージ
	public boolean isToMe(byte[] receiveBuffer) throws Exception{
		// 宛先IPアドレスのコピー先を作成
		...
		if(Arrays.equals(toIpAdd,InetAddress.getLocalHost().getAddress()))
				return true;
		else return false;
	}
